...
def test_ai_parse_returns_empty_when_unconfigured(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(v3_recon, "ai_configured", lambda: False)
    p = str(tmp_path / "x.xlsx")
    write_workbook(p, [("s", [ALIPAY_HDR], [["2026-08-01", "1", "n", "S1",
                                             "店1", "甲", "POSM", "PMS", "1"]])])
    assert v3_recon.ai_parse_daily_cols(p) == {}
...
def test_ai_parse_daily_cols_and_parse(client, monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(v3_recon, "ai_configured", lambda: True)
    monkeypatch.setattr(v3_recon, "_chat", lambda prompt: fake)
    ...
    cols = v3_recon.ai_parse_daily_cols(p)
    ...
    db.commit()
    parsed = v3_recon.parse_daily_records(p, "2026-08", db=db, ai_cols=cols)
    db.close()
...
def test_ai_parse_invalid_mapping_returns_empty(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(v3_recon, "ai_configured", lambda: True)
    monkeypatch.setattr(v3_recon, "_chat",
...
def test_interpret_prompt_contains_key_data(client, tmp_path, monkeypatch):
    ...
    db.add(t)
    db.commit()
    prompt = v3_recon.build_interpret_prompt(db, t.id)
...
def test_unmatched_collected_in_parse(client, tmp_path):
    ...
    um = []
    parsed = v3_recon.parse_daily_records(p, "2026-08", db=db, unmatched=um)
    db.close()
    assert parsed is not None
...
def test_report_ai_notes_in_explain_column(client, monkeypatch):
    ...
    wb = v3_recon.build_report(db, t.id, "管理员")
    # 员工×日 说明列
    ws = wb["员工×日对账明细"]
