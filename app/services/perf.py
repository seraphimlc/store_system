...
def _amount_schedule():
    """解析按月奖额规则 "2026-09=1250,..." → [(month, amount)]（升序）。"""
    raw = getattr(_get_settings(), "bonus_amount_schedule", "") or ""
    out = []
    for part in raw.split(","):
        part = part.strip()
        if "=" not in part:
